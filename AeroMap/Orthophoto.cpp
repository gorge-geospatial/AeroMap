...
void Orthophoto::get_orthophoto_vars(QStringList args)
{
	// Add orthophoto variables to argument list.
	// 
	
	args.push_back("-co");
	args.push_back(XString::Format("TILED=%s", arg.orthophoto_no_tiled ? "NO" : "YES").c_str());
	args.push_back("-co");
	args.push_back(XString::Format("COMPRESS=%s", arg.orthophoto_compression.c_str()).c_str());
	args.push_back("-co");
	args.push_back(XString::Format("PREDICTOR=%d", (arg.orthophoto_compression == "LZW" || arg.orthophoto_compression == "DEFLATE") ? 2 : 1).c_str());
	args.push_back("-co");
...
void Orthophoto::build_overviews(XString orthophoto_file)
{
	Logger::Write(__FUNCTION__, "Building Overviews");
	...
	args.push_back("BIGTIFF_OVERVIEW");
	args.push_back("IF_SAFER");
	args.push_back("--config");
	args.push_back("COMPRESS_OVERVIEW");
	args.push_back("JPEG");
	args.push_back(orthophoto_file.c_str());
	args.push_back("2");
	args.push_back("4");
	args.push_back("8");
	args.push_back("16");
	AeroLib::RunProgram(tree.prog_gdal_addo, args);
...
void Orthophoto::generate_png(XString orthophoto_file, XString output_file, SizeType outsize)
{
...
void Orthophoto::generate_kmz(XString orthophoto_file, XString output_file, SizeType outsize)
{
...
void Orthophoto::post_orthophoto_steps(XString bounds_file_path, XString orthophoto_file, XString orthophoto_tiles_dir)
{
	if (arg.crop > 0 || arg.boundary)
	{
		Cropper::crop(bounds_file_path, orthophoto_file /*, get_orthophoto_vars(args)*/, true/*, warp_options = ['-dstalpha']*/);
	}

    //if args.build_overviews and not args.cog:
    //    build_overviews(orthophoto_file)

	if (arg.orthophoto_png)
        generate_png(orthophoto_file);

    if (arg.orthophoto_kmz)
...
//        Logger::Write(__FUNCTION__, "Cannot mask raster, %s does not exist" % input_raster)
//        return
//
//    if not os.path.exists(vector_mask):
//        Logger::Write(__FUNCTION__, "Cannot mask raster, %s does not exist" % vector_mask)
//        return
//
...
//                else:
//                    Logger::Write(__FUNCTION__, "%s does not have an alpha band, cannot blend cutline!" % input_raster)
//
...
//    if not os.path.exists(input_raster):
//        Logger::Write(__FUNCTION__, "Cannot feather raster, %s does not exist" % input_raster)
//        return
//
...
//                np.multiply(alpha_band, dist_t, out=alpha_band, casting="unsafe")
//            else:
//                Logger::Write(__FUNCTION__, "%s does not have an alpha band, cannot feather raster!" % input_raster)
//
//        with rasterio.open(output_raster, 'w', BIGTIFF="IF_SAFER", **rast.profile) as dst:
//            dst.colorinterp = rast.colorinterp
//            dst.write(out_image)
...
//            Logger::Write(__FUNCTION__, "%s does not exist. Will skip from merged orthophoto." % o)
//            continue
//        if not io.file_exists(c):
//            Logger::Write(__FUNCTION__, "%s does not exist. Will skip from merged orthophoto." % c)
//            continue
//        inputs.append((o, c))
//
//    if len(inputs) == 0:
//        Logger::Write(__FUNCTION__, "No input orthophotos, skipping merge.")
//        return
//
